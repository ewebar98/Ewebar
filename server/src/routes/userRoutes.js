...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Read file as base64 for vision AI
    ...
    let prompt;
    if (reqType === "jamb") {
      prompt = `This is a JAMB UTME result slip. Extract ALL candidate details. Return ONLY valid JSON with these fields:
{
  "examType": "JAMB",
  ...
  "examNumber": string,
  "examYear": string,
  "gender": string (if visible),
  "dateOfBirth": string (if visible),
  "stateOfOrigin": string (if visible),
  "score": number (total aggregate score),
  "subjects": [ { "name": string, "grade": string } ]
}
For subjects, the grade should be the numeric score. Extract every subject shown.`;
    } else {
      const examLabel = reqType ? reqType.toUpperCase() : "O-Level";
      prompt = `This is a ${examLabel} result slip (could be WAEC, NECO, GCE, or NABTEB). Extract ALL candidate details. Return ONLY valid JSON with these fields:
{
  "examType": string (WAEC, NECO, GCE, or NABTEB),
  "candidateFullName": string,
  "examYear": string,
  "sittingType": string (e.g. May/June, Nov/Dec, June/July),
  "examNumber": string,
  "registrationNumber": string (if visible),
  "centerNumber": string (if visible),
  "schoolNumber": string (if visible),
  "dateOfBirth": string (if visible),
  "gender": string (if visible),
  "subjects": [ { "name": string, "grade": string } ]
}
For subjects, the grade should be the letter grade (e.g. A1, B2, C4, F9). Extract every subject shown.`;
    }
    ...
    try {
      ...
      if (ocrResult.subjects) {
        ocrResult.subjects = ocrResult.subjects.map((s) => ({
          name: s.name,
          grade: String(s.grade ?? s.score ?? ""),
        }));
      }
    } catch (parseErr) {
      res.status(500);
